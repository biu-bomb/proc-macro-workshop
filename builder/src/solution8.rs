...
pub(super) fn solution(fields: &crate::common::FieldsType, builder_ident: &syn::Ident, origin_ident: &syn::Ident) -> syn::Result<proc_macro2::TokenStream> {
...
fn solution2(fields: &crate::common::FieldsType, builder_ident: &syn::Ident, origin_ident: &syn::Ident) -> syn::Result<proc_macro2::TokenStream> {
...
fn solution3(fields: &crate::common::FieldsType, builder_ident: &syn::Ident, _: &syn::Ident) -> syn::Result<proc_macro2::TokenStream> {
    let setter_stream_vec_result: syn::Result<Vec<proc_macro2::TokenStream>> = fields.iter().map(|f| {
        let ident = &f.ident;
        let ty = &f.ty;
        if let std::option::Option::Some(inner_type) = crate::common::option_type_with_ident(ty, "Option") {
            ...
            if let std::option::Option::Some(ref each_method_ident) = crate::common::each_method_result(f)? {
...
fn solution45(fields: &crate::common::FieldsType, builder_ident: &syn::Ident, origin_ident: &syn::Ident) -> syn::Result<proc_macro2::TokenStream> {
