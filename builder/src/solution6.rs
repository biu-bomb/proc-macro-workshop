...
fn solution2(fields: &crate::common::FieldsType, builder_ident: &syn::Ident, origin_ident: &syn::Ident) -> proc_macro2::TokenStream {
    let builder_fields_stream_vec: Vec<_> = fields.iter().map(|f| {
        let ident = &f.ident;
        let ty = &f.ty;
...
fn solution3(fields: &crate::common::FieldsType, builder_ident: &syn::Ident, _origin_ident: &syn::Ident) -> proc_macro2::TokenStream {
    let setter_method_stream_vec: Vec<_> = fields.iter().map(|f| {
        let ident = &f.ident;
        let ty = &f.ty;
        match crate::common::option_type(ty) {
            std::option::Option::Some(inner_type) => {
...
fn solution45(fields: &crate::common::FieldsType, builder_ident: &syn::Ident, origin_ident: &syn::Ident) -> proc_macro2::TokenStream {
    let construct_if_stream_vec: Vec<_> = fields.iter().map(|f| {
        let ident = &f.ident;
        let ty = &f.ty;
        ...
        std::option::Option::None
    }).filter(|e| e.is_some()).collect();
    let construct_instance_stream_vec: Vec<_> = fields.iter().map(|f| {
        let ident = &f.ident;
        let ty = &f.ty;
        match crate::common::option_type(ty) {
